...
#ifndef COMPONENTS_TEST_UTIL_PROTO_MATCHER_H_
...
class ProtoMatcher {
 public:
  ProtoMatcher(
      ...
      std::function<bool(const MessageT&, const MessageT&)> message_comparator)
 ...
 private:
  void Describe(std::ostream* os, std::string_view explanation) const {
...
template <typename MessageT>
::testing::PolymorphicMatcher<ProtoMatcher<MessageT>> EqualsProto(
    const MessageT& message) {
...
template <typename MessageT>
::testing::PolymorphicMatcher<ProtoMatcher<MessageT>> EquivalentProto(
    const MessageT& message) {
  std::function<bool(const MessageT&, const MessageT&)> comparator =
...
template <typename MessageT>
::testing::PolymorphicMatcher<ProtoMatcher<MessageT>> ApproximatelyEqualsProto(
    const MessageT& message) {
  std::function<bool(const MessageT&, const MessageT&)> comparator =
...
template <typename MessageT>
::testing::PolymorphicMatcher<ProtoMatcher<MessageT>>
...
#endif  // COMPONENTS_TEST_UTIL_PROTO_MATCHER_H_
