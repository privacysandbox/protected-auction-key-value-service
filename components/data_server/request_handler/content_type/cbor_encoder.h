...
// Handles CBOR encoding/decoding for V2 API requests/responses
class CborV2EncoderDecoder : public V2EncoderDecoder {
 public:
  CborV2EncoderDecoder() = default;

  absl::StatusOr<std::string> EncodeV2GetValuesResponse(
      v2::GetValuesResponse& response_proto) const override;

  // Returns a serialized CBOR list of serialized CBOR partition outputs,
  // as per
