...
TEST(AstTest, Value) {
  ValueNode value("A");
  EXPECT_EQ(Eval<KVSetView>(value, Lookup), Lookup("A"));
  ValueNode value2("B");
  EXPECT_EQ(Eval<KVSetView>(value2, Lookup), Lookup("B"));
  ValueNode value3("C");
  EXPECT_EQ(Eval<KVSetView>(value3, Lookup), Lookup("C"));
  ValueNode value4("D");
  EXPECT_EQ(Eval<KVSetView>(value4, Lookup), Lookup("D"));
  ValueNode value5("E");
  EXPECT_EQ(Eval<KVSetView>(value5, Lookup), Lookup("E"));
}
...
TEST(AstTest, Union) {
  ...
  EXPECT_EQ(Eval<KVSetView>(op, Lookup), expected);
}
...
TEST(AstTest, Difference) {
  ...
  EXPECT_EQ(Eval<KVSetView>(op2, Lookup), expected2);
}
...
TEST(AstTest, All) {
  ...
  EXPECT_EQ(Eval<KVSetView>(center, Lookup), expected);
}
...
TEST(ASTEvalTest, VerifyValueNodeEvaluation) {
  {
    ...
    EXPECT_TRUE(Eval<KVSetView>(root, Lookup).empty());
  }
  ValueNode root("A");
  EXPECT_THAT(Eval<KVSetView>(root, Lookup),
